...
const initPuppeteerPool = () => {
    if (global.pp) {
        global.pp.drain().then(() => global.pp.clear());
    }
    const opt = {
        ...
        idleTimeoutMillis: 1000 * 60 * 10, //如果一个实例 10分钟 都没访问就关掉他
        evictionRunIntervalMillis: 1000 * 60 * 1, //每 1分钟 检查一次 实例的访问状态
        maxUses: 1000, //自定义的属性：每一个 实例 最大可重用次数。
        validator: (instance) => {
